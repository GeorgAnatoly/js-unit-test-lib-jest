...
class Assert {
    ...
    static isNull(input) {
        return test("isNull", () => {
            expect(Validation.isNull(input)).toBe(true);
    })
    }
    ...
    static arraysAreEqual(array, array2) {
        return test("arraysAreEqual", ()=> {
          expect(Arrays.compareArrays(array, array2)).toBe(true);
        })
    }
}
